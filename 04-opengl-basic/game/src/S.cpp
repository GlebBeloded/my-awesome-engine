...
tetris::S::S()
    ...
    data[2].move_to_coords(5, 11); // C
    data[3].move_to_coords(6, 11); // D
    position_iterator = 0;
}

void tetris::S::rotate() {
...
void tetris::S::render(eng::engine* e) {
    for (auto& tile : data) {
        tile.render(e);
    }
}

void tetris::S::move(eng::event e) {
    switch (e) {
...
void tetris::S::move(int x, int y) {
...
std::array<std::pair<int, int>, 4> tetris::S::coords_after_rotation(int r) {
...
void tetris::S::set_color(color c) {
    for (auto& tile : data)
        tile.set_color(c);
}
